...
impl Handler for ReceiveHandler {
    type Input = ServerMessage;
    type Output = (Task, Metadata);
    type Aux = (char, String, CommandParser);
    type Error = ReceiveError;

    type Receiver = mpsc::UnboundedReceiver<Self::Input>;
    type Sender = mpsc::UnboundedSender<Self::Output>;

    async fn new(msg_rx: Self::Receiver, task_tx: Self::Sender, aux: Self::Aux) -> Self {
        let (prefix, twitch_name, parser) = aux;
        Self {
            msg_rx,
            task_tx,
            prefix,
